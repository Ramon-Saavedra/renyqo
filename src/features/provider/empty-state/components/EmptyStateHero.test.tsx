...
describe("EmptyStateHero", () => {
  it("renders the welcome pill text", () => {
    ...
    expect(screen.getByText("Willkommen bei renyqo")).toBeInstanceOf(HTMLElement);
  });
  ...
  it("renders the h1 title", () => {
    ...
    expect(
      screen.getByRole("heading", { level: 1, name: "Weniger sortieren. Klarer vermieten." }),
  ...
  it("renders the lead paragraph", () => {
    ...
    expect(screen.getByText("Lege dein Mietobjekt einmal sauber an.")).toBeInstanceOf(HTMLElement);
  });

  it("renders the CTA link with the correct href", () => {
    render(<EmptyStateHero {...DEFAULT_PROPS} />);

    const link = screen.getByRole("link", { name: /Erstes Mietobjekt anlegen/ });
  ...
  it("renders the trust pill text", () => {
    ...
    expect(screen.getByText("Du kannst jederzeit als Entwurf speichern.")).toBeInstanceOf(HTMLElement);
  });
});
