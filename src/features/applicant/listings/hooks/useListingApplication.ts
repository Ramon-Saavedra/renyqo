...
export type ListingApplicationState =
  ...
  | {
      readonly status: "eligibility-rejected";
...
function messageFor(error: unknown): string {
  ...
  if (error.status === 403)
    return "Du kannst dich derzeit nicht auf dieses Objekt bewerben.";
  return "Die Bewerbung konnte nicht gesendet werden.";
}

export function useListingApplication(id: string) {
  const [state, setState] = useState<ListingApplicationState>({
    status: "idle",
  });
  const isSubmittingRef = useRef(false);
  const submit = async (): Promise<"duplicate" | "success" | void> => {
