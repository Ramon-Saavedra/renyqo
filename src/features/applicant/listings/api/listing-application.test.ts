...
describe("applyToListing", () => {
  it.each(["ACTIVE", "WAITING"] as const)(
    "returns backend %s status",
    async (status) => {
      vi.mocked(apiPost).mockResolvedValue({ ...application, status });
      await expect(applyToListing("listing-1")).resolves.toMatchObject({
        status,
      });
    },
  );
  ...
  it("maps a 422 eligibility rejection", async () => {
    vi.mocked(apiPost).mockRejectedValue(
      new ApiError(
        422,
        "Applicant is not eligible for this listing",
        "http",
        null,
        {
          canApply: false,
